...
class ParticleText(ShowBase):
    ...
    def start_move(self):
        ...
        # Sequence(
        #     Parallel(
        #         LerpFunc(self.scatter_text2, duration=1, fromData=0, toData=400, blendType='easeInOut'),
        #         self.np.colorScaleInterval(1, (1, 1, 1, 0), blendType='easeInOut')
        #     ),
        #     Wait(1),
        #     Parallel(
        #         LerpFunc(self.scatter_text2, duration=1, fromData=400, toData=0, blendType='easeOut'),
    ...
    def get_dest(self, x, y):
        nx = x / 406
        ny = y / 40

        px = self.perlin.snoise2(nx, ny * 1.5) - 0.5
        py = self.perlin.snoise2(nx * 3, ny) - 0.5
        # px = self.perlin.snoise2(nx, ny * 2)
        # py = self.perlin.snoise2(nx * 2, ny)
            ...
            for i in range(w):
                ...
                if not (cell[0] == 0 and cell[1] == 0 and cell[2] == 0):
                    for _ in range(num_per_cel):
                        ...
                        # vdata_values.extend([x / 406 * 600, 0, y / 40 * 59])
